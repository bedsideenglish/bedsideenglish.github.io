...
class ModelInterviewGenerationTests(unittest.TestCase):
    def test_release_has_five_unique_cases_and_ten_unique_voices(self) -> None:
        records = load_records()
        self.assertEqual(5, len(records))
        self.assertEqual(5, len({record.case_id for record in records}))
        voices = [name for record in records for name in (record.item["voice"]["doctor"], record.item["voice"]["patient"])]
        self.assertEqual(10, len(set(voices)))
